...
class _CustomThemeSwitchState extends State<CustomThemeSwitch>
  ...
  // Builds the icon inside the toggle handle.
  Widget _buildIcon() {
    // If `isOn` is true, show the dark icon (or a provided custom one).
    // If `isOn` is false, show the light icon (or a provided custom one).
    return widget.isOn
        ? (widget.iconDark ??
            Icon(Icons.nights_stay, color: widget.darkIconColor))
        : (widget.iconLight ??
